...
const gReady = ref(false);
const gSignedIn = ref(false);
...
async function signOutGoogle() {
    try {
        await axios.post('/api/integrations/google/revoke');
    } catch {}
...
async function listAllCalendars() {
    ...
    do {
        const resp = await callGapi(() => gapi.client.calendar.calendarList.list({ maxResults: 250, pageToken }));
...
async function syncGoogleCalendar() {
    ...
    try {
        ...
        events.value = [...events.value.filter((e) => !(isGoogle(e) && inRange(e))), ...flattened];
...
async function callGapi<T extends GoogleApiResponse>(fn: () => Promise<T>): Promise<T> {
    try {
        ...
        const is401 = err?.status === 401 || err?.result?.error?.code === 401 || /invalid_credentials|unauthorized/i.test(String(err?.message ?? ''));
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="h-full w-full">
            <a-alert v-if="loading" type="info" show-icon message="Loading events..." class="latte-alert mb-3" />

            <!-- Calendar card with Google auth chips -->
            <div class="ring-latte-200/70 h-full overflow-hidden bg-white/95 shadow-[0_8px_30px_rgb(0,0,0,0.06)] ring-1">
                <!-- Top bar (optional: shows Google auth state) -->
                <div class="flex items-center justify-between border-b border-black/5 px-3 py-2">
                    <div class="text-latte-500 text-sm">FullCalendar</div>
                    <div class="flex items-center gap-2">
                        ...
                        <a-button size="small" v-if="!gSignedIn" @click="connectGoogle" :disabled="!gReady"> Connect Google </a-button>

                        <a-button size="small" v-else @click="signOutGoogle"> Disconnect </a-button>

                        <a-button type="primary" size="small" @click="syncGoogleCalendar" :disabled="!gReady || !gSignedIn">
            ...
            <!-- Drawer/Modal -->
            <a-modal v-model:open="isOpen" :title="isEdit ? 'Edit Event' : 'New Event'" :footer="null" destroy-on-close width="560px">
                <a-form ref="formRef" :model="form" layout="vertical">
                    <a-form-item name="title" label="Title" :rules="[{ required: true, message: 'Title is required' }]"
                        ><a-input v-model:value="form.title" placeholder="e.g., Team Standup"
                    /></a-form-item>
                    <a-form-item label="Description" name="description"
                        ><a-textarea v-model:value="form.description" auto-size placeholder="Optional notes..."
                    /></a-form-item>
                            ...
                            <a-select v-model:value="form.color" :options="colorOptions" :dropdownStyle="{ padding: '6px' }">
                                ...
                                <template #tagRender="{ value, label, closable, onClose }">
                                    <a-tag
                                        ...
                                        @close="onClose"
                                        class="rounded-full"
                                        :style="{ background: value, color: '#fff', border: 'none' }"
                                        >{{ label }}</a-tag
                                    >
                                </template>
                            </a-select>
